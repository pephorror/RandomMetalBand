...
class App(tk.Tk):
    ...
    def lastfm_charts(self):
        ...
        if answer == "yes":
            ...
            root = tk.Tk()
            root.title('LastFM Metal Charts. In the left, by genre. In the right, global charts.')
            # CREATE A NEW WINDOW. IN THE LEFT, CHARTS BY GENRE. TO THE RIGHT, GLOBAL CHARTS
            df = df.groupby("GENRE").apply(lambda x: x.sort_values("SCROBBLES", ascending = False))
            df2 = df.sort_values("SCROBBLES", ascending = False)
            frame = tk.Frame(root)
            frame.pack(side = "left", fill = "both")
            tabla = Table(frame, dataframe=df)
            tabla.show()
            frame2 = tk.Frame(root)
            frame2.pack(side = "right", fill = "both")
            tabla2 = Table(frame2, dataframe=df2)
            tabla2.show()
            root.mainloop()
